...
export async function POST(request: NextRequest) {
  try {
    ...
    const tempDir = path.join(tmpDir, 'trade-plans');
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }

    const tempPath = path.join(tempDir, `plan-${symbol.toLowerCase()}-${Date.now()}.png`);

    // Generate the professional trade plan image
    await generateTradePlanImage(tempPath, {
...
async function generateTradePlanImage(outputPath: string, data: TradePlanData): Promise<void> {
  ...
  panelSVG += `
    ...
    <text x="${panelX + panelWidth / 2}" y="${chartTop + 55}" font-family="Arial, sans-serif" 
          font-size="14" fill="#94a3b8" text-anchor="middle">${data.symbol}</text>
  `;
  ...
  for (const level of levels) {
    panelSVG += `
      ...
      <text x="${panelX + 22}" y="${level.y + 32}" font-family="Arial, sans-serif" 
            font-size="9" fill="rgba(255,255,255,0.35)">${level.labelAr}</text>
      <text x="${panelX + panelWidth - 22}" y="${level.y + 30}" font-family="monospace" 
            font-size="16" font-weight="bold" fill="#ffffff" text-anchor="end">${level.value}</text>
    `;
  }
  ...
  let headerSVG = `
    ...
    <text x="80" y="38" font-family="Arial, sans-serif" font-size="18" font-weight="bold" fill="#e2e8f0" letter-spacing="1">TradeX AI</text>
    <text x="80" y="56" font-family="Arial, sans-serif" font-size="11" fill="#64748b">Professional Trading Agent</text>
    <text x="${width - 20}" y="38" font-family="monospace" font-size="11" fill="#64748b" text-anchor="end">${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</text>
    <text x="${width - 20}" y="56" font-family="monospace" font-size="10" fill="#475569" text-anchor="end">${data.symbol}</text>
  `;
  ...
  let footerSVG = `
    ...
    <text x="20" y="${height - 28}" font-family="Arial, sans-serif" font-size="10" fill="rgba(255,255,255,0.15)">TradeX AI</text>
    <text x="${width / 2}" y="${height - 15}" font-family="Arial, sans-serif" font-size="9" fill="rgba(239, 68, 68, 0.3)" text-anchor="middle">For educational purposes only</text>
  `;
